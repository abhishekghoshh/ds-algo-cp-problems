...
**Topic:** `array` | **File:** `com/problems/array/MinimumNumberOfSwapsToMakeTheStringBalanced.java`
...
Check this solution again We will be a little greedy here The total number of unmatched opening brackets at the end will tell us how many swaps are necessary to balance the string so, we keep track the number of unmatched closing brackets. Each swap can fix two unmatched brackets, so the minimum number of swaps required is (unmatched+1)/2
