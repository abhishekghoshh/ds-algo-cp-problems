...
same two pointers approach we are taking from j=1 as nums[0] is already in its place when we find a new element at that time, we are only changing nums[i]
...
two pointers approach we will increase the right until it found a new element for the last element like 4, right will go till n and stops left+1 will be the position of the new element for the last element right is n, that's why it terminates, so there is no new element
...
brute force approach for every element it will check it's next element
