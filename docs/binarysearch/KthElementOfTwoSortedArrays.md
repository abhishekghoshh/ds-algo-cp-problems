...
**Topic:** `binarysearch` | **File:** `com/problems/binarysearch/KthElementOfTwoSortedArrays.java`
...
Same as previous just this code is much cleaner TODO best solution using binary search approach
...
Binary search approach we can also use the heap approach but as the arrays are in sorted order so we can leverage this thing time complexity O(log(min(n1,n2))) space complexity O(1)
...
Merge approach but without using the array time complexity O(k) space complexity O(1)
...
Merge approach in merge sort time complexity O(n1+n2) space complexity O(n1+n2)
