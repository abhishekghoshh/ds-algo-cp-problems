...
**Topic:** `binarysearch` | **File:** `com/problems/binarysearch/MedianOfRowWiseSortedMatrix.java`
...
Https://www.geeksforgeeks.org/find-median-row-wise-sorted-matrix/#:~:text=Simple%20Method%3A%20The%20simplest%20method,O(r*c). study one more time
...
Brute force approach if r is row and c is column and n=r*c then time complexity is O(n+n*log(n)) O(n) to put it in list O(n*log(n)) to sort the list space complexity O(n) for the list
