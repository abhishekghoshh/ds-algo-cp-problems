...
**Topic:** `binarysearch` | **File:** `com/problems/binarysearch/MedianOfTwoSortedArray.java`
...
Check this solution one more time binary search approach we will try to find the partition from both of the array such that the numbers in the left portion and the number in the right portion follows this rule count(nums1[0..left1])+count(nums2[0..left2]) will be equal to count(nums1[left1..])+count(nums2[left2..]) also nums1[left1]<=nums2[left2+1] and nums2[left2]<=nums1[left1+1]
...
Time complexity is O(log(min(n1,n2))) space complexity O(1) same as previous just here we are not creating any other array we are using extra variable merge approach but without using the array space complexity O(1)
...
Merge algorithm in merge sort approach in merge sort time complexity O(n1+n2) space complexity O(n1+n2)
