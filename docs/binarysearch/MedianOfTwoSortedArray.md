...
- [📄 Coding Ninjas](https://www.codingninjas.com/codestudio/problems/985294)

## 🎥 Solution Links

- [▶ YouTube](https://www.youtube.com/watch?v=NTop3VTjmxk&list=PLgUwDviBIf0p4ozDR_kJJkONnb1wdx2Ma&index=66)
- [📄 takeUforward](https://takeuforward.org/data-structure/median-of-two-sorted-arrays-of-different-sizes/)

## 📝 Problem Statement

Given two sorted arrays nums1 and nums2, return the median of the two sorted arrays. Complexity: O(log(m+n)).
...
same as previous just it looks clean if n1 is bigger swap the arrays: apply binary search: calculate l1, l2, r1 and r2;
...
check this solution one more time binary search approach we will try to find the partition from both of the array such that the numbers in the left portion and the number in the right portion follows this rule count(nums1[0..left1])+count(nums2[0..left2]) will be equal to count(nums1[left1..])+count(nums2[left2..]) also nums1[left1]<=nums2[left2+1] and nums2[left2]<=nums1[left1+1] we are assuming that the first array is always less in size low = 0 means we are not taking any elements from 1st array as the element of the 1st array are higher than the second array so element in the first array will fall after the median high=n1 means we are not taking any element from the first array as the element of the first array are less than the 2nd array so all the element of the first array will fall before the median position here we are not finding the index rather we are trying to find the cut that's why we took high = n1 rather than n1-1 we are taking median = (n1 + n2 + 1) / 2 because it will work for both it will work for both, when (n1+n2) is even or odd lets say n1=5 and n2=6 then (n1+n2+1)/2 => 6 and we know that 6th element will be median and by our code we know that by our cutting logic 6th element will be in the left portion that's why we will return Math.max(l1,l2) when n1=6 and n2=6 then also (n1+n2+1)/2 =>6 and we know here median will be (6th element + 7th element)/2, and we know by our cut logic in the left side there will be 6 elements and in the right side also there will be 6 elements so we will return max from the left and min from the right cut1 means we are picking cut1 elements from the firsts array 0 means no element from first array we are assuming that n1 is less than or equal to n2 cut1 can be n1 means all the element of of 1st array is less than all the elements of 2nd array and if n1<n2 then essentially we have consumed the first array and the median can be found in the second array else n1==n2 then the median will be (nums1[last] + nums2[first])/2 cut2 can only be n2 when n1 equal n2 and all the elements of 2nd array is less than all the element of 1st array we know one thing that as the array is sorted then l1 will always be less than r1 and l2 will always be less than r2, but in the median l1 will also be less than the r2 and l2 will also be less than r1, that will make a perfect
...
same as previous just here we are not creating any other array we are using extra variable merge approach but without using the array
