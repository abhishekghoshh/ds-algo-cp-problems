...
**Topic:** `binarysearchtree` | **File:** `com/problems/binarysearchtree/ConvertNormalBinarySearchTreeToBalanced.java`
...
This approach is pretty optimized first we will get the imbalanced tree and get the inorder traversal of the tree. The inOrder traversal always gives us the sorted list. Then with this inOrder traversal we can create a balanced BST it is very easy to create a balanced bst if the inorder is given TODO rather creating a list of Integer we could use a list of Tree node we could directly save the tree node in the list and while rebuilding the Balanced Binary Search Tree we could use the same nodes
