...
**Topic:** `binarysearchtree` | **File:** `com/problems/binarysearchtree/IntroductionToBinarySearchTree.java`
...
Left < root < right generally BST does not contain any duplicate, but we can change that we can change add an extra variable like count or frequency to check the current number of occurrences of any number in BST
