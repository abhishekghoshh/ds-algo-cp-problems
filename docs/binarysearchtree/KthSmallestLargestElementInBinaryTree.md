...
kth largest we will visit the nodes from right to left then we can skip the counting operation if the count has already reached k then will go no more we will return from here every time we visit a non-null node we will increment the counter if the count is k then we will set the answer
...
similar to the previous type, but here we will use two class level variables only / if root is null or k < 0 then we will return else we will do inorder we will decrement k and check if it is 0 or not
...
kth smallest we know the inorder of bst will give us the sorted list we will keep a counter, and when the counter is k then we will store the value if the count has already reached k then will go no more we will return from here every time we visit a non-null node we will increment the counter if the count is k then we will set the answer
