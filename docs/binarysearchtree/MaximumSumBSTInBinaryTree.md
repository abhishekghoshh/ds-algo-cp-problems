...
Find the maximum sum BST subtree in a binary tree.
...
in the previous type, we have called check BST method twice for two scenarios. but somehow if we can use only one function call to check if the subtree is BST. with that information, if we can also calculate the current node is a part of the BST or not, then the overall problem will be optimized.

so we will check left and right is an individual subtree or not at first. then we need to know that if the highest of the left subtree is lesser than the root and the lowest of the right subtree is greater than the node or not. so we will use a custom data structure to hold all this information.

we will return null if any of the subtree is non BST or the root does not satisfy the range criteria here we will return sum 0 and isBST false and as this is null, so we will return the min as INF and max -INF so the null will be satisfied whether it will be left or the right side checking if both are BST or not if any of the subtree is null that means they are not BST, so the current node is also not BST or the root node is not following the range condition then also it is not a BST at this point, the current node is BST and the left and right subtree is also BST updating the max sum returning the new pair with updated range and sum
