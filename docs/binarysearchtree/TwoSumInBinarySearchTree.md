...
study it one more time we will check all node value with the whole tree we have added (k != 2 * node.data) because otherwise find would match with the node again which is a wrong answer it is a simple search function to find k in BST
...
so rather than storing the whole list, we can iterate once from front and once from back, and meanwhile we can calculate. we know how to iterate a BST, so we can similarly derive how can iterate the list from back we will use two pointers, front and end for traversing the BST it is same as two sum problem in an array
...
this is similar to two sum problems in an array, so we can store the inorder traversal in a list which will be sorted and then we can apply same two sum problem in the inorder list
