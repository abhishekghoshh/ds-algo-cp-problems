# TwoSumInBinarySearchTree

**Topic:** `binarysearchtree` | **File:** `com/problems/binarysearchtree/TwoSumInBinarySearchTree.java`

## Problem Links

- [📄 LeetCode](https://leetcode.com/problems/two-sum-iv-input-is-a-bst/)
- [📄 GeeksforGeeks](https://www.geeksforgeeks.org/problems/find-a-pair-with-given-target-in-bst/1)
- [📄 Coding Ninjas](https://www.naukri.com/code360/problems/pair-sum-in-bst._920493)
...
All three approaches are efficient, TODO study it one more time
...
We will check all node value with the whole tree it is a simple search function to find k in BST so rather than storing the whole list, we can iterate once from front and once from back, and meanwhile we can calculate. we know how to iterate a BST, so we can similarly derive how can iterate the list from back
...
We will use two pointers, front and end for traversing the BST it is same as two sum problem in an array this is similar to two sum problems in an array, so we can store the inorder traversal in a list which will be sorted and then we can apply same two sum problem in the inorder list
