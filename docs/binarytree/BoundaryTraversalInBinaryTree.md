...
recursive approach as for right view it will be reversed, so we are creating a stack later we will iterate through it and add the node in the final array as we don't need the root, we will not iterate till empty we will loop until the size is 1 we are also not taking the leaf node as it will be added in the bottom view we are only adding for the leaf node
