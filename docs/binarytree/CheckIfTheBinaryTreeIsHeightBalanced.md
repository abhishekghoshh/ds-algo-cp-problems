# CheckIfTheBinaryTreeIsHeightBalanced
...
- [📄 Coding Ninjas](https://www.naukri.com/code360/problems/975497)
...
- [📄 takeUforward](https://takeuforward.org/data-structure/check-if-the-binary-tree-is-balanced-binary-tree/)
...
little optimized it will calculate height in the same time and return that, but in between we will tweak the implementation we will check if the abs(left-height - right-height) > 1 then that is not balanced, and we will return -1 now the tree can return -1 as height so after call height for left and right subtree we will first check if it is not 1, then only we will do the remaining things left subtree is not balanced right subtree is not balanced if the height difference is greater than 1 then the tree is not balanced now we will return the current height
...
first, it will check left height and right height, and if that are balanced or not then it will go to its left and right
