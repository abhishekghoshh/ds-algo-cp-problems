...
**Topic:** `binarytree` | **File:** `com/problems/binarytree/CheckIfTheBinaryTreeIsHeightBalanced.java`
...
Little optimized it will calculate height in the same time and return that, but in between we will tweak the implementation we will check if the abs(left-height - right-height) > 1 then that is not balanced, and we will return -1 now the tree can return -1 as height so after call height for left and right subtree we will first check if it is not 1, then only we will do the remaining things
