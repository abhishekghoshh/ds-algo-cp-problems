...
check it one more time properly here the first we need to go left, then we need to print root then the right subtree with iteration using stack observation: if the node is null that might be the ending of a leaf node and parent nodes might be in the stack we are going as left side as possible because in the recursion also we went as left side as possible at this point, the node is null; now we backtrack from the stack.

current stack[top] will have the left node, so for this node null will be it's left child it will be the root, and if it is having any right child, then we will explore that also we will store the left most node the left node may or may not have any right node if it has any right node, then in the next iteration it will again go till its left furthest node
...
With recursion first left subtree then root, then right subtree
