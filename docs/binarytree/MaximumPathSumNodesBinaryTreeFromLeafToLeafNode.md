...
this is brute force but it is an efficient approach as we are taking from the leaf, so we cannot discard any value if this is null, then it will return 0 if this is a leaf node, then it will return the node value at this point, either of sides must be non-zero if both of its left and right children are non-empty, that means it is a proper parent node with at least one child in its both sides.

we could check now if the path is the max path or not lastly, we will return the max out of left and right
