...
Find the most profitable path in a tree.
...
we will do the dfs with the backtracking first, we will convert the edges into an adjacency list if alice has reached the leaf node, then we can directly return the amount associated with that node if bob has reached 0, then we will go till alice reached to any leaf node if the amount is -INF, then that means there is no possible path if bob reached any leaf node other than 0, then he has to return we will move alice and bob simultaneously first, we will set the extract the cost but at last we will again reset it
