...
- [📄 LeetCode](https://leetcode.com/problems/inverse-coin-change/description/)

## 📝 Problem Statement

Given coin denominations and a count, find the maximum amount achievable.
...
this is a hard problem, need to understand the logic if you have understood the coin change problem, then this is the inverse of that this is a optimal solution, but it is not easy to understand dp of coin means by now the coin can be formed in this many ways if that number + 1 is equal to the number of ways from the numWays array then the coin is also present now we need to update the dp array for all the coins that can be formed using this coin
