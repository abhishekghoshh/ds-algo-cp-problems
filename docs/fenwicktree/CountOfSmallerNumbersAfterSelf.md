...
same as the inversion count problem using fenwick tree but here we will do differently we will not create the fenwick tree first and decrease the count rather we will start from clean tree we do it from the last and increase the frequency of num by one and the last we reverse the ans we will also tackle negative numbers here first we will be calculating the max number and will create an array for holding all of this as fenwick tree is 1 indexed, so we will use offset and -min+1 not -min here we will use an empty tree we will start from the last, and we will increase the frequency by one here we will not need the freq array even it is a clever approach as we are skipping the fenwick tree building time and freq array time and space now here we will take num from last and increase the frequency by 1
