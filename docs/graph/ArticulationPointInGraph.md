# ArticulationPointInGraph

**Topic:** `graph`  

## 🔗 Problem Links

- [📄 GeeksforGeeks](https://practice.geeksforgeeks.org/problems/articulation-point-1/1)

## 🎥 Solution Links

- [▶ YouTube](https://www.youtube.com/watch?v=j1QDfU21iZk&list=PLgUwDviBIf0oE3gA41TKO2H5bHpPd7fzn&index=56)
- [📄 takeUforward](https://takeuforward.org/data-structure/articulation-point-in-graph-g-56/)
...
check the video once again this draws intuition from the bridges in graph problem we will 3 arrays here we will use time of insertion, low and mark array timeOfInsertion array is the first time when then we come to any node low is the lowest time of insertion apart from the parent we will start the dfs from all the unvisited nodes if mark is 1 then read the comments again and change it accordingly we are marking the start as an articulation point here we are not adding it to the answer here directly, as start can be articulation point, for multiple components, so we are just marking it here this part is different from the bridges of graph if the end is already visited, then we will be taking from timeOfInsertion[end].

because we are removing the start node hypothetically, if we remove the start node, then we cannot take its low time, so we will take timeOfInsertion if it is a starting point, and it has multiple children, then also starting point can be articulation point
