...
the intuition is, to find the minimum distance we have to relax all the edges, but how many times? if there are n nodes, then at max all n nodes will be at a straight line, so at max that will have n-1 edges and to relax n-1 edges, we will at max need n-1 iterations.

we will relax all edges n-1 times. so, ideally, after n-1 operation, all the nodes will have minimum distance. then we will again try to relax all the edges, if at this time any edge tries to relax then we will know that we are fallen into a negative cycle we will relax all the edges n-1 times because if all the vertices are in straight line, then only they have at max n-1 edges dis[source] is Integer.MAX_VALUE means source node itself is not relaxed if there is any negative cycle, then some edge will relax again, so we will try to relax one more time
