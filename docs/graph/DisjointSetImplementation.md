...
Disjoint set using rank It will also use Path compression along the way It uses a recursive approach to find and do the union method union rank means adding one edge if 3 and 7 are in the same component or not adding 3 and 7 into the same component if the node is itself parent, then it is the ultimate parent of the component recursively, we will find the ultimate parent path compression by setting the ultimate parent if they remove the next line then all node will just connect to its upper parent
