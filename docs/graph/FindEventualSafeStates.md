...
**Topic:** `graph` | **File:** `com/problems/graph/FindEventualSafeStates.java`
...
Using DFS same as previous, but here we will directly add the nodes to the safe nodes list, we will not use extra loop
...
Using dfs with adjacency matrix and an single visited matrix
