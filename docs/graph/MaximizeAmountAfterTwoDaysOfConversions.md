...
Maximize amount after series of currency conversions.
...
using recursion and dfs We will create a map of initial currency and final currency. However, we can also convert final currency to initial currency with (1/rate). However, if we do this, then we will have an endless loop so to prevent that we will use a visited array and along with storing the currency, we will also store the index.

We could also store the rates, but we can derive it from the 'rates' array easily we can use the conversion of the first day if (day == 0) we can use the conversion of the second day for both of the days storing initial -> final storing final -> initial
