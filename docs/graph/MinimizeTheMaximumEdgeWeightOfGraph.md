# MinimizeTheMaximumEdgeWeightOfGraph

**Topic:** `graph` | **File:** `com/problems/graph/MinimizeTheMaximumEdgeWeightOfGraph.java`
...
- [📄 LeetCode](https://leetcode.com/problems/minimize-the-maximum-edge-weight-of-graph/description/)
...
Check it later it is very close to be solved todo my intention is to reverse the edges after reversing the edge we will try to relax an edge now as the edges are reversed so we will check the indegree now // we are reversing the edge here now outgoing nodes of source means ingoing nodes then apply dijkstra algorithm
