...
**Topic:** `graph` | **File:** `com/problems/graph/MostStonesRemovedWithSameRowOrColumn.java`
...
On a 2D plane, we place n stones at some integer coordinate points. Each coordinate point may have at most one stone. A stone can be removed if it shares either the same row or the same column as another stone that has not been removed. Given an array of stones of length n where stones[i] = [xi, yi] represents the location of the i'th stone, return the largest possible number of stones that can be removed. our work it a group all the stones that are in the same row and column, ideally they are connected by this. so if we remove all the stones from that group except 1, then our work is done then there will be no stone which will be in the same group the answer is = (componentA -1) + (componentB -1) + (componentC -1) + (componentD -1) .... ans = (componentA + componentB + componentC + componentD...) - (1 + 1 + 1 + 1 ...) ans = total summation of component size - number of components ans = number of all stones - number of components
