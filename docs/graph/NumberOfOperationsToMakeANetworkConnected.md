...
**Topic:** `graph` | **File:** `com/problems/graph/NumberOfOperationsToMakeANetworkConnected.java`
...
There are n computers numbered from 0 to n - 1 connected by ethernet cables connections forming a network where connections{i} = {ai, bi} represents a connection between computers ai and bi. Any computer can reach any other computer directly or indirectly through the network. You are given an initial computer network connections. You can extract certain cables between two directly connected computers, and place them between any pair of disconnected computers to make them directly connected. Return the minimum number of times you need to do this to make all the computers connected. If it is not possible, return -1. We can directly use union find data structure here if there are x components then we need x-1 wires to connect if we have exactly x-1 or more unused ropes, then we can use that ropes to connect others
