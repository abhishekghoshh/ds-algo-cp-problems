...
- [📄 LeetCode](https://leetcode.com/problems/properties-graph/description/)
...
check other submissions also using bit-set/boolean array instead of array converting properties array to bitset array now we will initialize parent and rank now we will check for all the nodes i and j already in the same component, like (2,3) and (2,4) is already calculated then 3,4 is not necessary now we will check for intersect
...
we can apply little optimization here instead of creating an adjacency list and then saving to that we can directly apply union find and group the components then at last we will return the number of component now we will initialize parent and rank now we will check for all the nodes point-I will be the parent of point=J finding the parent iteratively
...
simplest approach first we will create the graph then we will use dfs to find we are create the adjacency list first one optimization here we are going i from 0..n-1 and j from i+1..n-1 so that if 1-3 is computed then it should not compute 3-1 again then using a visited array we are creating the components
