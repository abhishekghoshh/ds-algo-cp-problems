...
at means after k+1 times of adding, we will get the item like for the series 6 5 3 2 for getting 0th element we have to add k+1 element in the heap the max size of heap can be k+1 after all the iteration, there should be some elements in the heap; we will add them one by one to the answer
