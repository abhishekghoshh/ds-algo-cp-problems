...
**Topic:** `heap` | **File:** `com/problems/heap/TaskScheduler.java`
...
Check again later using heap using the freq TODO same as previous just that we will do it in different way Study it one more time from nick white video
...
Explain this solution, it is best approach
...
Optimized approach using heap this will be similar to how a processor will do and we will add the job freq and time at which the job would be taken again we will also use a clock and increment the clock by 1 in every iteration and we will use 2 data structure one heap and one queue heap for for pulling the most freq item, once we will pull any item and e
