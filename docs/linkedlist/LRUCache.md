...
check leetcode submission for a better solution We can also create an array in place of map if the key is integer, and we know the key range which is very good for leetcode but in real world scenario it will not help and using generics may look over engineering, but it will work better if we have a dynamic key value pair based on situation
