...
if start > end, then we will return -1 now we will partition the array start to (mid-1) and mid and (mid+1) to end if target is in mid then we will return mid if it is lesser, then we will go to the left else go to the right section
