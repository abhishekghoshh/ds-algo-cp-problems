...
- [📄 Coding Ninjas](https://www.naukri.com/code360/problems/758958)

## 🎥 Solution Links

- [▶ YouTube](https://www.youtube.com/watch?v=YK78FU5Ffjw&list=PLgUwDviBIf0p4ozDR_kJJkONnb1wdx2Ma&index=51)
- [▶ YouTube](https://www.youtube.com/watch?v=f2ic2Rsc9pU&list=PLgUwDviBIf0p4ozDR_kJJkONnb1wdx2Ma&index=52)
- [▶ YouTube](https://www.youtube.com/watch?v=zC4D7cuaYzo&list=PL_z_8CaSLPWdbOTog8Jxk9XOjzUs3egMP&index=7)
- [▶ YouTube](https://www.youtube.com/watch?v=XZQKgcjbogo&list=PL_z_8CaSLPWdbOTog8Jxk9XOjzUs3egMP&index=7)
- [▶ YouTube](https://www.youtube.com/watch?v=EnRciMd08_g&list=PL_z_8CaSLPWdbOTog8Jxk9XOjzUs3egMP&index=8)
- [📄 takeUforward](https://takeuforward.org/data-structure/print-all-permutations-of-a-string-array/)
...
using backtracking in-place-array with two optimizations if in the current iteration we have already visited this character, then we will skip this
...
explain this in the interview assuming there are some duplicates, we will be creating a boolean map to so that don't encounter any duplicate element similar to a type1 solution we could have used a boolean array instead of hashset we will only consider one element at a label if it is not used previously that's why we are using a set to keep track of the characters which we have, already used in this label lets say the string is abac, we are now 0th index,
...
same as type previous type3 in place of a char array here it is an int array this is using backtracking as we are doing it in place
...
assuming all characters are distinct, choosing one character or element to be the first string array needs to convert in to array. if it's an int array, then we can take the List of Integers for bucket this is using backtracking as we are doing it in place Let say the string is abcd and the starting index is 0.

Now we are choosing c to be the first, so we swap it with index, so string will be cbad now again we will start recursion from (starting index + 1) gain after all the computation we will replace it back
...
this is a brute force approach assuming all characters are distinct, here we are using a set at every point we recursion, we will add one unique element
...
assuming All elements are distinct, only compatible with string as we can perform string.substring(0, i) + string.substring(i + 1) with string only suppose the string is 'abcd' and previous is empty. so we will first consider 'a', bcd then b,acd then c,abd then now for 'a','bcd', previous is a and current string is 'bcd' we will follow the same 'ab','cd' then ac,bd then ad,bc
