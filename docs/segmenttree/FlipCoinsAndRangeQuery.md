...
Range flip and count queries using lazy segment tree.
...
in update query range of coins will be flipped. we will store the number of heads only flip means rangeCount - countOfHead we will use a boolean array for lazy index if false means no need to flip then true means flip even times of flip means no flip This is a problem of sum segment tree but as we are having range updates, so we are using lazy propagation here update the previous remaining update and propagate that later we can do the current operation node count - head coin count flipping the head counts if low!=high then it has children so we can propagate the addition down our main query operation if we are using modulo if the number is more than 1, ideally it should not occur update the node when you visit it propagate the update to downwards only for the complete overlap we will change the node and propagate if we flip twice, then the head count remains the same if before flip the head count was x then after flip the count will be (r-l+1) - x later we can update the current update high-low+1 will give the count of the range for which the node is responsible if low!=high then it has children, so we can propagate the addition down actual addition for current addition flip the coin heads will be equal to tails which is node-heads if the node is responsible for more than one node so can propagate the addition
