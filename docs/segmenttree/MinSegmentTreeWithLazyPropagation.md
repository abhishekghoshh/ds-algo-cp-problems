...
check the lazy sum propagation for better understanding for range minimum query instead of tree[index] += (high - low + 1) * val; the tree[index] will be updated by val only tree[index] += val; update the previous remaining update and propagate that later we can do the current operation if the both child is updated by some value then the root node also updated by the same value if low!=high then it has children so we can propagate the addition down our main query operation update the node when you visit it propagate the update to downwards only for the complete overlap we will change the node and propagate update the previous remaining update and propagate that later we can update the current update if the both children are updated by some value, then the root node also updated by if low!=high then it has children, so we can propagate the addition down actual addition for current addition update the current node by the addition if the node is responsible for more than one node so can propagate the addition
