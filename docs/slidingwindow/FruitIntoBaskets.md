...
- [📄 Coding Ninjas](https://www.naukri.com/code360/problems/fruits-and-baskets_985356)
...
best possible solution, same as type5 just it is less complicated to understand easy to understand and implement we have 3 cases current fruit is exactly prev type of fruit current fruit is previous to previous type of fruit current fruit is a completely new fruit we will take two pointer one for previous another for previous to previous fruit also we will store the previous fruit count if there is a new type of fruit then we will set that count to 1 because the current fruit will become my new prev fruit we will change the pointers accordingly we will use 2 variables prev and prev2 for storing the fruits which are count of the current series with prev and prev2 type of fruits if the current fruit is not the previous fruit then we have 2 cases either it is prev2 or different fruit if it is prev2 then the series should continue but if it is a different fruit then we need the count of prevFruit [which we can find using a loop] we will now update all the variables
...
same as previous but here we will use the array as freq map creating the frequency array we will go through the array and add the fruits one by one it is a new fruit if type is greater than 2 then we will shrink the window it is the last fruit, so we will decrease the type
...
optimized approach using sliding window this is max toys problem which is same as the max fruits problem we will go through the array and add the fruits one by one it is a new fruit if type is greater than 2 then we will shrink the window it is the last fruit, so we will decrease the type it is a new toy it is the last toy, so we will decrease the type
