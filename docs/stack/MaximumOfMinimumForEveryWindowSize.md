...
You are given an integer array arr[], the&nbsp;task is to find the maximum of minimum values for every window size k where&nbsp;1&le; k &le; arr.size().
For each window size k, consider all contiguous subarrays of length k, determine the minimum elem
...
brute force approach first we will fix a window size (1,n) we will compute the minimum in every window k is the size of the window we will compute the maximum of the minimum of all windows of size k we will compute the minimum of the current window taking the maximum for all window minimum for window of size k
