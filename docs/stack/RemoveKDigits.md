...
**Topic:** `stack` | **File:** `com/problems/stack/RemoveKDigits.java`
...
Same as type2 just optimized here we are mostly using the array as stack
...
Optimized solution using stack, we certainly know one thing if the integers are 2 5 4 5 9 1 then the lowest number can be possible with these integers, is 123559 todo the lowest number will be monotonically increasing, so we go from left to right and try to maintain stack of monotonic increasing if the number is 254591 and we need to choose between first 5 or first 4 then we will certainly choose the 5 to exclude because no matter what will be the remaining number if we exclude 5 it will be 24 and if we exclude 4 it will be 25 todo we go from left to right and maintain a monotonic stack
