...
Design a data structure supporting add and search with wildcard '.'.
...
using trie iteratively going through the word and adding the node if we have reached the end of the word, then we will just check if the current node is the end of the word if the node is null, then continue if the node is not null, then search for the next character
