...
public class NQueenProblem {
	...
	// It is the same as the previous type.
	// However, it if we see closely the time to check if the queen placement is safe or not
	// is taking O(3n), so if we can check if the left row and both left upper and left lower diagonal
	// has any queen or not, then we the code will be optimized
	...
	private static boolean isSafeToPlace2(int r, int c, int n, int[] leftRow, int[] lowerDiagonal, int[] upperDiagonal) {
		return (leftRow[r] == 0) && (lowerDiagonal[r + c] == 0) && (upperDiagonal[n - 1 + c - r] == 0);
	}
	...
	private static boolean isSafeToPlace(char[][] board, int row, int column) {
		...
		// we do not need to check the right because that cell will be placed after this
		return true;
	}
