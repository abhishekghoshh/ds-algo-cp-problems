...
public class DecodeWays {
    ...
    public static int numDecodings2(String s) {
        ...
        int[] dp = new int[n];
        Arrays.fill(dp, -1);
        return numDecodings1(0, n, arr, dp);
    }
    ...
    public static int numDecodings2(int i, int n, char[] arr, int[] dp) {
        ...
        if (i + 1 <= n - 1) {
            ...
            if (num >= 1 && num <= 26) count += numDecodings2(i + 2, n, arr, dp);
        }
        return dp[i] = count;
    }
    ...
    public static int numDecodings1(int i, int n, char[] arr, int[] dp) {
        // if i == n means we have exhausted the entire string, so we can use this decoding, hence returning 1
        if (i == n) return 1;
        int count = 0;
        int num = arr[i] - '0';
        // return 0 if the current index on 0
        if (num == 0) return 0;
        count += numDecodings1(i + 1, n, arr, dp);
        // checking if we can take the next character or not
        if (i + 1 <= n - 1) {
            num = (num * 10) + (arr[i + 1] - '0');
            // if the num is in range [1,26] then only we will consider it
            if (num >= 1 && num <= 26) count += numDecodings1(i + 2, n, arr, dp);
        }
        return count;
    }
}
