...
public class LongestIncreasingSubsequence {
    ...
    // rather we will store the nums in a length array
    // let's think about the brute force solution.
    // We will have to find all the increasing subsequence
    // and find the longest among them. However, that will take time and space,
    // so if we can store all the sequences in the same array.
    // Now if the new element is greater than the last element, then we will push the
    // element in the length array, else will try to find where it can be placed.
    // We will always try to minimize the array elements
    private static void type6() {
        ...
        lengths[0] = Integer.MIN_VALUE;
        int len = 0;
        for (int num : nums) {
            // if the current num is greater than the last number on LIS,
            // then we will add the num to len(LIS) + 1
            if (num > lengths[len]) {
                lengths[++len] = num;
            } else {
                // we will try to find a suitable place for the current num.
    ...
    private static void type2() {
        ...
        int ans = lengthOfLIS2(0, -1, n, nums, dp);
        System.out.println(ans);
    }
    ...
    private static void type1() {
        int[] nums = {10, 9, 2, 5, 3, 7, 101, 18};
        int n = nums.length;
        int ans = lengthOfLIS1(0, -1, n, nums);
        System.out.println(ans);
    }
