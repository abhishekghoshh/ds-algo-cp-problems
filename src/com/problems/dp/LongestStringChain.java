    ...
    public static int longestStrChain2(String[] words) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int prev = 0; prev < i; prev++) {
                ...
                // Here we will also check two other things. We will take advantage of the sorting nature here
                // if prev word length is still lesser than curr word + 1, then we will skip the prev.
                if (prevWord.length + 1 < currWord.length) continue;
                // If prev word length + 1 is bigger than the current length, then we can directly break the inner loop.
                // As the words are sorted by their length, and so if the prev word is already longer, then the next words of
                // the current prev will also be longer. So we can break the inner loop
                if (prevWord.length + 1 > currWord.length) break;
                // We will check if the current word is a one-letter extra word or not. However,
